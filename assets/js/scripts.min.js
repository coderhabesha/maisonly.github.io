...
document.querySelectorAll(".feature-card,.stat-card").forEach((e) => {
  ((e.style.opacity = "0"),
...
toggle.addEventListener("click", () => {
  const e = menu.classList.toggle("open");
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    const t = document.getElementById("name").value.trim(),
      ...
      n = document.getElementById("role").value;
    if (!t || !a || !n) return;
    const { error: o } = await supabase
      .from("waitlist")
      .insert([{ name: t, email: a, role: n }]);
    if (o)
      return (
    ...
    const s = "https://forms.gle/faNkDNyr5DWmUFWW6",
      r = "https://forms.gle/j2tSD7F1EjgwMei4A";
    window.location.href = "student" === n ? r : s;
  });
